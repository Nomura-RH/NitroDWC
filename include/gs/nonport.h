...
extern "C" {
    ...
    void gsMemMgrDestroy(void);
    void gsMemMgrDumpStats(void);
    void gsMemMgrDumpAllocations(void);
    void gsMemMgrValidateMemoryPool(void);
...
gsi_time current_time(void);
gsi_time current_time_hires(void);
...
gsi_u32 gsiGetBroadcastIP(void);
    ...
    int gsiStartThread(
        GSThreadFunc aThreadFunc,
        gsi_u32 theStackSize,
        void *arg,
    ...
    GSISemaphoreID gsiCreateSemaphore(
        gsi_i32 theInitialCount,
        gsi_i32 theMaxCount,
        char *theName
    );

    gsi_u32 gsiWaitForSemaphore(
        GSISemaphoreID theSemaphore,
        gsi_u32 theTimeoutMs
    );

    void gsiReleaseSemaphore(
        GSISemaphoreID theSemaphore,
        gsi_i32 theReleaseCount
    );
...
gsi_i8 *gsXxteaEncrypt(
    const gsi_i8 *iStr,
    gsi_i32 iLength,
    gsi_i8 key[XXTEA_KEY_SIZE],
    gsi_i32 *oLength
);

gsi_i8 *gsXxteaDecrypt(
    const gsi_i8 *iStr,
    gsi_i32 iLength,
    gsi_i8 key[XXTEA_KEY_SIZE],
    gsi_i32 *oLength
